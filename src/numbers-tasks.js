...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const v1Length = Math.sqrt(x1 * x1 + y1 * y1);
  const v2Length = Math.sqrt(x2 * x2 + y2 * y2);
...
function getFibonacciNumber(index) {
  const v1 = (1 + Math.sqrt(5)) / 2;
  const v2 = (1 - Math.sqrt(5)) / 2;
  return Math.round((v1 ** index - v2 ** index) / Math.sqrt(5));
}
